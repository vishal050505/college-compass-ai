          ...
          <div className="grid grid-cols-3 bg-slate-900 text-white">
            ...
            {[college1, college2].map((c) => (
              <div key={c.id} className="p-5 text-center">
                ...
                <div className="text-slate-400 text-sm mt-1">{c.location} · {c.type}</div>
              </div>
            ))}
          </div>
              ...
              {rows.map((row, i) => {
                ...
                return (
                  <tr key={row.key} className={i % 2 === 0 ? "bg-white" : "bg-slate-50"}>
                    <td className="p-4 font-bold text-slate-600 text-sm border-r border-slate-100">{row.label}</td>
                    <Cell value={row.format(college1[row.key])} winner={winner === "left"} />
                    <Cell value={row.format(college2[row.key])} winner={winner === "right"} />
                  </tr>
                );
              })}
                      ...
                      <td key={c.id} className="p-5 text-center">
                        <div className={`inline-flex flex-col items-center px-4 py-2 rounded-xl ${isWinner ? "bg-emerald-100 border border-emerald-200" : "bg-slate-100"}`}>
                          ...
                          <span className="text-xs text-slate-500 mt-0.5">
                            Wins {score}/{rows.filter((r) => typeof college1[r.key] === "number").length} metrics
                          </span>
                        </div>
                      </td>
                    );
